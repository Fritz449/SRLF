...
class A3CContinuousTrainer(FFContinuous):
    ...
    def create_internal(self):
        ...
        self.loss = -tf.reduce_mean(log_p * self.targets["advantage"])
        self.entropy = tf.reduce_mean(tf.log(self.action_stds*np.sqrt(2*np.pi*np.e)))
